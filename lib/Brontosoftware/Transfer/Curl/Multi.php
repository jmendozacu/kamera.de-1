 ...
 */
class Brontosoftware_Transfer_Curl_Multi
{
    protected $_multi;
    protected $_requests;
    ...
    {
        $this->_multi = $multi ? $multi : new Brontosoftware_StandardResource_Proxy('curl_multi_');
     ...
     *
     * @param Brontosoftware_Transfer_Curl_Request $request
     * @return self
     */
    public function add(Brontosoftware_Transfer_Curl_Request $request)
    {
     ...
     * @return self
     */
    public function remove(Brontosoftware_Transfer_Curl_Request $request)
    {
    ...
    protected function _scanAndReap()
    {
        while ($info = $this->_multi->infoRead()) {
            ...
            if ($result !== CURLE_OK) {
                ...
                $request->error(new Brontosoftware_Transfer_Exception($message, $result, $request));
            } else {
                $content = $this->_multi->getcontent($handle);
                $info = $request->getHandle()->getinfo();
                $request->complete(new Brontosoftware_Transfer_Curl_Response($content, $info));
            }
            $this->remove($request);
        }
    }
