 ...
 */
class Brontosoftware_DataObject
{
    protected $_data;
     ...
     * @return Brontosoftware_Functional_Option
     */
    protected function _safe($camelized)
    {
        if (array_key_exists($camelized, $this->_data)) {
            return new Brontosoftware_Functional_Some($this->_data[$camelized]);
        }
    ...
    protected function _camelizedValue($name)
    {
        if (preg_match('/^([^A-Z0-9]+).+/', $name, $match)) {
            ...
            return array($match[1], $this->_underscore ? Brontosoftware_Utils::underscore($modified) : $modified);
        }
        return array("", "");
    }
    ...
    /**
     * Brontosoftware_Serialized Object's should always be able to deserialize
     *
     * @return array
     */
