 ...
 *
 * For the original:
 * @see src/Bronto/Magento/Core/Catalog/CategoryResolverGeneric.php
 */

class Brontosoftware_Magento_Core_Catalog_CategoryResolverGeneric implements Brontosoftware_Magento_Core_Catalog_CategoryResolverInterface
{
    ...
    public function resolve($branches)
    {
        if ($this->_selection == 'all') {
            ...
            return implode('', array_map(array($this, 'encapsulate'), $output));
        } else {
            $categories = array();
            foreach ($branches as $branch) {
    ...
    {
        return implode($this->_delimiter, array_map(array($this, 'format'), $branch));
    }
    ...
    public function specify($categories)
    {
        if ($this->_selection == 'all' || empty($categories)) {
            ...
            usort($categories, array($this, '_sortCategory'));
    ...
    protected function _sortCategory($categoryA, $categoryB)
    {
        ...
        if ($specificityA == $specificityB) {
            ...
            } else if ($broadnessA > $broadnessB) {
                return $this->_broadness == 'most' ? -1 : 1;
            } else {
                return $this->_broadness == 'most' ? 1 : -1;
            }
        } else if ($specificityA < 2 || $specificityB < 2) {
